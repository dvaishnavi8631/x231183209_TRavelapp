...
def registerUser(request):
    if request.method == "POST":
        ...
        if len(error) != 0:
            ...
            print(error)
            return render(
                request, "Travel2gether/registration_form.html", {"error": error}
            )
        else:
...
def search(request):
    ...
    for query in queries:
        results["posts"] = results["posts"] | User.objects.filter(
    ...
    for query2 in queries:
        results2["posts"] = results2["posts"] | User.objects.filter(
            first_name__icontains=query2
        )
    ...
    for query3 in queries:
        results3["posts"] = results3["posts"] | User.objects.filter(
            last_name__icontains=query3
        )
